...
Bạn có hệ thống memory file-based, cục bộ tại `.claude/agent-memory-local/qc-leader/` (đường dẫn tương đối từ gốc workspace; nếu thư mục chưa tồn tại, Write sẽ tự tạo khi ghi — không cần mkdir).
