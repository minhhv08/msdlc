...
1. **Idempotent check** — tìm story gắn ticket này (trường `Ticket:` trong các `.claude/stories/*/requirement.md`), rồi rẽ theo ba trường hợp:
   - Story tồn tại **và đã có `adr.md`** → bỏ qua (đã xử lý), sang ticket kế.
   - Story tồn tại nhưng **chưa có `adr.md`** (lượt trước architect fail/dở dang) → **resume trên story cũ**: giữ nguyên `{id}` và `requirement.md`, nhảy thẳng tới bước 3 (gọi architect). TUYỆT ĐỐI không cấp id mới — một ticket chỉ được gắn một story.
   - Chưa có story nào gắn ticket → tiếp bước 2.
...
4. **Ghi dấu duyệt vào ADR**: cập nhật header `adr.md` từ `Status: Proposed` thành `Status: Accepted · Duyệt: <hôm nay> (qua board — người kéo sang Approved)` nếu chưa `Accepted`. Việc người kéo thẻ sang Approved CHÍNH LÀ hành động duyệt; dấu này là tiền đề để `deliver-auto` chạy.
5. Gọi skill **`deliver-auto {id}`**. Skill đó tự lo chuyển `in-progress` ở Phase 1 và `review` ở Phase 5. **Không hỏi gate** ở đây — gate đã được người vượt bằng thao tác kéo thẻ sang Approved.
6. Nếu `deliver-auto` fail giữa chừng (dev/QC lỗi không tự fix nổi trong ngân sách) → log rõ story/ticket bị kẹt và lý do, **để nguyên ticket ở cột hiện tại** cho người xử lý; không retry trong cùng lượt poll và không tự kéo ticket lùi cột.
