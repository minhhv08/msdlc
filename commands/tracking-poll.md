...
- Có thư mục nhưng **không có `plan.md`** (task-planner chết giữa chừng), ticket ở `planning`/trung gian → **resume phân tích**: gọi lại Agent `task-planner` cho `{taskid}` → ghi `plan.md`. Giữ nguyên `{taskid}`, không claim lại.
